...
public class BankService {
	...
	*/
	...
	public AccountDto[] getAccountList(int userSeq) {
		 ...
		 AccountDto[] ad = new AccountDto[count];
		 for (int i=0 ; i < count ; i++ ) {
				if( accountList[i].getUserSeq()==userSeq )	{
					ad[i]=new AccountDto(accountList[i].getAccountSeq(),accountList[i].getAccountNumber(),accountList[i].getBalance(),accountList[i].getUserSeq());
